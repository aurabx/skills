## Aurabox Skills Agents

This project contains reusable skill modules that AI coding agents load into context when processing medical imaging tasks. Each skill is self-contained in a directory with a `SKILL.md` configuration file that describes its purpose and dependencies.

### Available Skills

| Skill | Purpose |
|-------|---------|
| `aurabox-rest-api` | Python/TypeScript clients for Aurabox platform (patients, cases, studies) |
| `dicom-processing` | DICOM file operations with pydicom and DCMTK |
| `dicomweb-protocol` | DICOMweb client implementation (WADO-RS, STOW-RS, QIDO-RS) |
| `medical-imaging-pipelines` | Imaging preprocessing and ML dataset preparation pipelines |

### How Skills Work

1. Each skill loads only its `SKILL.md` description during agent startup (zero cost)
2. When a prompt matches a skill's description, the full content loads into context automatically
3. No manual invocation required - matching triggers loading
4. Multiple skills can coexist without context bloat

### Installation

Copy desired skill directories to `.claude/skills/` for project-specific access or `~/.claude/skills/` for global access. Restart the agent after installation.
